...
private enum class AnimeProviderOption(
...
fun DeveloperScreenRoute(
    ...
    DeveloperScreen(
        ...
        onAnimeProviderSelected = viewModel::setAnimeProvider
    )
}
...
fun DeveloperScreen(
    ...
    onAnimeProviderSelected: (AnimeProvider) -> Unit
) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.developer_anime_provider),
                style = MaterialTheme.typography.titleMedium,
                modifier = Modifier.padding(horizontal = ScreenPadding, vertical = ElementSpacing)
            )

            AnimeProviderOption.entries.forEach { option ->
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                        .heightIn(min = MinTouchTarget)
                        .clickable { onAnimeProviderSelected(option.provider) }
                        .padding(horizontal = ScreenPadding),
                    verticalAlignment = Alignment.CenterVertically
                ) {
                    RadioButton(
                        selected = uiState.animeProvider == option.provider,
                        onClick = null
                    )
                    Text(
